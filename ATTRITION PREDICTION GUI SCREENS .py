...
def get_entry_data():
    ...
    data.head(5)
    data.to_csv('C:\\Users\\srira\\Downloads\\HR-Analytics-and-Employee-Attrition-Prediction-master\\HR-Analytics-and-Employee-Attrition-Prediction-master\\Datasets\\LabelEncoded_CleanData.csv')
...
gui.title('EMPLOYEE ATTRITION GUI by sriram ')
...
l29.grid(row=27, column=2,padx=10,pady=2)
el29 = Entry(gui)
el29.grid(row=27, column=3)

action_button = Button(gui)
action_button.configure(text='Submit',fg="black",bg="green", command=get_entry_data)
action_button.grid(row=28, column=2,padx=10,pady=2)
